...
class BinningTokenizer:
    ...
    def tokenize(
        ...
        Args:
            ...
            actions: Optional normalized actions in the range [-1, 1], shaped
                (action_horizon, action_dim). When provided, actions are discretized
                into RT-2/OpenVLA-style bins and appended after ``Action:``.
    ...
    def _encode_actions(self, actions: np.ndarray) -> np.ndarray:
        ...
        bins = np.linspace(-1, 1, self._n_bins + 1)[:-1]
        action_tokens = np.digitize(np.clip(actions, -1, 1), bins) - 1
        return np.clip(action_tokens, 0, self._n_bins - 1).astype(np.int32).reshape(-1)
...
class FSQTokenizer:
    ...
    def _encode_actions(self, actions: np.ndarray) -> np.ndarray:
        try:
            device = jax.devices("cpu")[0]
            with jax.default_device(device):
                action_tokens, _ = self._tokenize_fn(self._params, np.asarray(actions, dtype=np.float32)[None, ...])
            return np.asarray(jax.device_get(action_tokens[0]), dtype=np.int32).reshape(-1)
        except Exception as e:
            raise RuntimeError(f"Error encoding FSQ actions: {e}") from e
