...
def get_ua_cg_module(cfg: Any, llm_dim: int, patches_per_image: int, num_images: int) -> UACGOFTModule:
    """
    Load the optional UA-CG-OFT module for uncertainty-aware adaptive flow-matching inference.
    """
    ua_cg_module = UACGOFTModule(
        llm_dim=llm_dim,
        action_dim=ACTION_DIM,
        patches_per_image=patches_per_image,
        num_images=num_images,
        hidden_dim=getattr(cfg, "ua_cg_hidden_dim", None),
        uncertainty_loss_weight=getattr(cfg, "ua_uncertainty_loss_weight", 1.0),
        grounding_loss_weight=getattr(cfg, "cg_grounding_loss_weight", 1.0),
        grounding_dice_weight=getattr(cfg, "cg_grounding_dice_weight", 1.0),
        contrastive_loss_weight=getattr(cfg, "cg_contrastive_loss_weight", 1.0),
        equivariance_loss_weight=getattr(cfg, "cg_equivariance_loss_weight", 1.0),
        uncertainty_error_scale=getattr(cfg, "ua_uncertainty_error_scale", 1.0),
        contrastive_temperature=getattr(cfg, "cg_contrastive_temperature", 0.07),
    ).to(DEVICE)
    ua_cg_module = ua_cg_module.to(torch.bfloat16).to(DEVICE)
    ua_cg_module.eval()

    checkpoint_path = find_checkpoint_file(cfg.pretrained_checkpoint, "ua_cg_module")
    state_dict = load_component_state_dict(checkpoint_path)
    ua_cg_module.load_state_dict(state_dict)
    return ua_cg_module
...
def get_vla_action(
    ...
    ua_cg_module: Optional[torch.nn.Module] = None,
    ...
    Args:
        ...
        ua_cg_module: Optional UA-CG module for adaptive flow-matching inference
