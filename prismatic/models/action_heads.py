...
class L1RegressionActionHead(nn.Module):
    ...
    def predict_action(self, actions_hidden_states):
        ...
        action_tokens = actions_hidden_states.shape[1]
        assert action_tokens % self.action_dim == 0, "Action hidden states must divide evenly by action_dim."
        rearranged_actions_hidden_states = actions_hidden_states.reshape(batch_size, action_tokens // self.action_dim, -1)
...
class DiffusionActionHead(nn.Module):
    ...
    def sample_noisy_actions(self, ground_truth_actions):
        ...
        noise = torch.randn_like(ground_truth_actions)  # (B, chunk_len, action_dim)
    ...
    def predict_noise(self, actions_hidden_states):
        ...
        action_tokens = actions_hidden_states.shape[1]
        assert action_tokens % self.action_dim == 0, "Action hidden states must divide evenly by action_dim."
        rearranged_actions_hidden_states = actions_hidden_states.reshape(batch_size, action_tokens // self.action_dim, -1)  # (batch_size, chunk_len, action_dim * hidden_dim)
...
class FlowMatchingActionHead(nn.Module):
    """
    Continuous action head trained with rectified flow matching.

    The VLA backbone conditions on an intermediate action state x_t and a continuous timestep t. This head predicts the
    velocity that transports x_t from random Gaussian noise toward the expert action chunk.
    """
    ...
    def __init__(
        ...
        hidden_dim=4096,
        action_dim=7,
        num_flow_steps_train=50,
        num_flow_steps_inference=8,
    ):
        super().__init__()
        self.action_dim = action_dim
        self.num_flow_steps_train = num_flow_steps_train
        self.num_flow_steps_inference = num_flow_steps_inference
    ...
    def sample_intermediate_actions(self, ground_truth_actions):
        """
        Samples x_0 ~ N(0, I), t ~ U[0, 1], and x_t = (1 - t) x_0 + t x_1.

        Returns the intermediate actions, timestep embeddings, and target velocity x_1 - x_0.
        """
        batch_size = ground_truth_actions.shape[0]
        device = ground_truth_actions.device
        dtype = ground_truth_actions.dtype

        start_noise = torch.randn_like(ground_truth_actions)
        timesteps = torch.rand(size=(batch_size,), device=device, dtype=dtype)
        timestep_view = timesteps.reshape(batch_size, 1, 1)
        intermediate_actions = (1.0 - timestep_view) * start_noise + timestep_view * ground_truth_actions
        target_velocity = ground_truth_actions - start_noise

        timestep_embeddings = self.time_encoder(timesteps.float()).to(dtype).to(device).unsqueeze(1)

        return dict(
            start_noise=start_noise,
            timesteps=timesteps,
            noisy_actions=intermediate_actions,
            flow_timestep_embeddings=timestep_embeddings,
            target_velocity=target_velocity,
        )

    def predict_velocity(self, actions_hidden_states):
        """
        Predicts a velocity field for each action in the chunk from conditioned action-token hidden states.
        """
        batch_size = actions_hidden_states.shape[0]
        action_tokens = actions_hidden_states.shape[1]
        assert action_tokens % self.action_dim == 0, "Action hidden states must divide evenly by action_dim."
        rearranged_actions_hidden_states = actions_hidden_states.reshape(batch_size, action_tokens // self.action_dim, -1)
        return self.velocity_predictor(rearranged_actions_hidden_states)
