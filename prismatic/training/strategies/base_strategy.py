                ...
                if overwatch.is_rank_zero():
                    ...
                    if len(datasets) > 1:
                        for ds in datasets:
                            ...
                            action_accuracy_ds = compute_token_accuracy(
                                predicted_token_ids[ds_mask],
                                ground_truth_token_ids[ds_mask],
                                mask=current_action_mask[ds_mask],
                            )
                            action_l1_loss_ds = compute_actions_l1_loss(
                                action_tokenizer,
                                predicted_token_ids[ds_mask],
                                ground_truth_token_ids[ds_mask],
                                mask=current_action_mask[ds_mask],
                            )
                            next_actions_accuracy_ds = compute_token_accuracy(
                                predicted_token_ids[ds_mask],
                                ground_truth_token_ids[ds_mask],
                                mask=next_actions_mask[ds_mask],
                            )
                            next_actions_l1_loss_ds = compute_actions_l1_loss(
                                action_tokenizer,
                                predicted_token_ids[ds_mask],
                                ground_truth_token_ids[ds_mask],
                                mask=next_actions_mask[ds_mask],
                            )
                            dataset_name = ds.decode() if isinstance(ds, bytes) else str(ds)
                            metrics.commit_for_dataset(
                                dataset_name=dataset_name,
                                action_accuracy=action_accuracy_ds,
                                l1_loss=action_l1_loss_ds,
                                next_actions_accuracy=next_actions_accuracy_ds,
                                next_actions_l1_loss=next_actions_l1_loss_ds,
                            )
